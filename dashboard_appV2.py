# dashboard_app.py - V5.8 (Based on V5.3) - Handle Errors + Fix Fig7 Hover Name
...
try:
    # Ensure logger is available before potential import errors
    ...
    try:
        ...
        class ChartData:
            ...
            # Simple vars fallback for model_dump if Pydantic v2 isn't used everywhere
            def model_dump(self, **kwargs): return {'metadata': vars(self.metadata) if self.metadata else {}, 'data': self.data}
    ...
    # Use basic print for fatal error if logger failed
    print(f"FATAL ERROR: Could not import core modules. Details: {e}")
    # Use st.error for Streamlit visibility
    st.error(f"Fatal Error: Could not import core modules. Dashboard cannot start. Details: {e}")
    CORE_IMPORTED = False
    # Define dummies to prevent further NameErrors if st.stop() doesn't halt immediately
    class DataPipeline: pass
    class ChartData: pass
    class ChartType(str, Enum): BAR = "bar"; LINE = "line"; PIE = "pie"; SCATTER = "scatter"; TABLE = "table"; COMBO = "combo"
...
# --- Basic Settings ---
app_version = "5.8" # Updated version reflecting fix
page_title = f"Client Dashboard (Direct DB V{app_version})"
...
def initialize_pipeline() -> Optional[DataPipeline]:
    ...
    try:
        ...
        if pipeline.engine is None:
            st.error("Failed to connect to the database during pipeline initialization.")
            logger.critical("DataPipeline engine is None after initialization.")
            return None
        logger.info("DataPipeline initialized successfully with DB engine.")
        return pipeline
    except Exception as e:
        st.error(f"Critical error initializing DataPipeline: {e}")
        logger.critical(f"Failed to create DataPipeline instance: {e}", exc_info=True)
        return None

pipeline: Optional[DataPipeline] = initialize_pipeline()
if not pipeline:
    st.warning("Data processing functionality is unavailable due to pipeline initialization failure.")
    st.stop()
...
def get_clients_from_db(_pipeline_ref) -> List[str]:
    ...
    if not pipeline or not pipeline.engine:
        logger.error("Cannot fetch clients, pipeline or engine is None.")
        return []
    table_to_query = 'sale_invoices'; client_id_col_name_in_config = 'client_id'; actual_client_id_col = 'client_id'
    if config and hasattr(config, 'required_columns') and isinstance(config.required_columns, dict) and \
       table_to_query in config.required_columns and isinstance(config.required_columns[table_to_query], list):
        try:
            client_id_col_index = config.required_columns[table_to_query].index(client_id_col_name_in_config)
            actual_client_id_col = config.required_columns[table_to_query][client_id_col_index]
            logger.info(f"Found client ID column '{actual_client_id_col}' from config.")
        except ValueError:
            logger.warning(f"'{client_id_col_name_in_config}' not found in config list for '{table_to_query}', using default '{actual_client_id_col}'.")
    else:
        logger.warning(f"Config definition missing, invalid, or '{table_to_query}' key missing/invalid. Using default client ID column '{actual_client_id_col}'.")
    ...
    try:
        with pipeline.engine.connect() as connection:
            client_df = pd.read_sql(query, connection)
            if not client_df.empty:
                actual_col_name_in_df = client_df.columns[0]
                client_ids = [str(cid) for cid in client_df[actual_col_name_in_df].unique() if pd.notna(cid)]
            else: client_ids = []
        logger.info(f"Found {len(client_ids)} distinct client IDs from DB.")
    except Exception as e:
        logger.error(f"Error executing client list query '{query}': {e}", exc_info=True)
        return []
    return client_ids
...
def run_pipeline_and_get_data(_pipeline_ref, client_id: int, lang: str) -> Optional[Dict[str, Optional[ChartData]]]:
    ...
    try:
        ...
        figure_preparers_config = { # Based on V5.3 - No fig8
            "fig1": {"func": pipeline._prepare_fig1_data, "inputs": [analysis_results.get('product_flow')]},
            "fig2": {"func": pipeline._prepare_fig2_data, "inputs": [analysis_results.get('product_flow')]},
            "fig3": {"func": pipeline._prepare_fig3_data, "inputs": [analysis_results.get('product_flow')]},
            "fig4": {"func": pipeline._prepare_fig4_data, "inputs": [analysis_results.get('product_flow')]},
            "fig5": {"func": pipeline._prepare_fig5_data, "inputs": [analysis_results.get('product_flow')]},
            "fig6": {"func": pipeline._prepare_fig6_data, "inputs": [analysis_results.get('pareto_data')]},
            "fig7": {"func": pipeline._prepare_fig7_data, "inputs": [analysis_results.get('product_flow')]},
            "fig9": {"func": pipeline._prepare_fig9_data, "inputs": [analysis_results.get('product_flow')]},
            "fig10": {"func": pipeline._prepare_fig10_data, "inputs": [analysis_results.get('stagnant_products')]},
            "fig11": {"func": pipeline._prepare_fig11_data, "inputs": [daily_forecast_results]},
            "fig12": {"func": pipeline._prepare_fig12_data, "inputs": [analysis_results.get('monthly_avg_invoice_ts'), monthly_forecast_results]},
            "fig13": {"func": pipeline._prepare_fig13_data, "inputs": [analysis_results.get('product_flow')]},
            "fig14": {"func": pipeline._prepare_fig14_data, "inputs": [pipeline.processed_data.get('sale_invoices_details')]},
            "fig15": {"func": pipeline._prepare_fig15_data, "inputs": [analysis_results.get('product_flow')]},
            "fig16": {"func": pipeline._prepare_fig16_data, "inputs": [analysis_results.get('pie_data')]},
        ...
        for fig_key, config_item in figure_preparers_config.items():
            ...
            if chart_data_obj and isinstance(chart_data_obj, ChartData):
                prepared_data[fig_key] = chart_data_obj
                log_msg_suffix = "(contains error)" if isinstance(chart_data_obj.data, dict) and chart_data_obj.data.get("error") else ""
                logger.debug(f"Successfully prepared {fig_key} (lang={lang}) {log_msg_suffix}")
            else: logger.warning(f"Preparer {fig_key} (lang={lang}) returned None or wrong type ({type(chart_data_obj)}). Storing None."); prepared_data[fig_key] = None
        logger.info(f"--- Finished on-demand pipeline run for Client ID: {client_id} (Lang: {lang}) ---")
        return prepared_data
    except Exception as e_pipe_run:
        st.error(f"Error running data pipeline for client {client_id}: {e_pipe_run}")
        logger.error(f"Failed pipeline run for client {client_id}: {e_pipe_run}", exc_info=True)
        return None
...
def find_column(df: pd.DataFrame, possibilities: List[Optional[str]], chart_title: str = "Chart", exclude: Optional[str] = None) -> Optional[str]:
    ...
    clean_possibilities = [p for p in possibilities if isinstance(p, str) and p]; df_cols_lower = {col.lower(): col for col in df.columns if isinstance(col, str)}
    # Ensure exclude is a list for easier checking
    if exclude is None: exclude = []
    elif not isinstance(exclude, list): exclude = [exclude]

    def check_and_return(col_name: Optional[str]) -> Optional[str]:
        if col_name and col_name not in exclude: return col_name; return None # Check against exclude list
...
def create_plotly_figure_replica(fig_key: str, chart_data_obj: Optional[ChartData], current_lang: str) -> Optional[go.Figure]:
    ...
    try:
        if not chart_data_obj or not isinstance(chart_data_obj, ChartData) or not chart_data_obj.metadata:
            st.warning(f"[{fig_key}] Invalid or missing ChartData object structure.")
            logger.warning(f"[{fig_key}] Invalid ChartData format or object is None.")
            return None
        ...
        if isinstance(data, dict) and data.get("error") is True:
            error_message = data.get("message", get_translation_streamlit(current_lang, 'error.no_data', "No data available"))
            logger.warning(f"[{fig_key}] Received error structure in data: {error_message}")
            st.warning(f"**{title}**: {error_message}")
            return None # Handled

        is_empty_data = data is None or (isinstance(data, (list, dict)) and not data)
        if is_empty_data:
             logger.warning(f"[{fig_key}] No chart data found (and not an error structure).");
             st.info(f"{title}: {get_translation_streamlit(current_lang, 'error.no_data', 'No Data Available')}")
             return None # Handled

        # --- Proceed with plotting ---
        chart_type_enum = metadata.get('chart_type', ChartType.TABLE); chart_type_str = str(chart_type_enum.value) if isinstance(chart_type_enum, Enum) else str(chart_type_enum).lower()
        ...
        trans_product_name_hover = get_translation_streamlit(current_lang, 'common.product_name', 'Product Name') # Specific translation for hover name

        if isinstance(data, dict) and chart_type_str == ChartType.PIE.value:
            if fig_key == 'fig16': # Dual Pie
                ...
                rev_data = data.get('revenue_data'); df_rev = pd.DataFrame(rev_data) if rev_data and isinstance(rev_data, list) else pd.DataFrame()
                ...
                else: st.info(f"{get_translation_streamlit(current_lang, 'error.no_data', 'No Data Available')} ({rev_title_trans})")
                prof_title_trans = get_translation_streamlit(current_lang, 'fig16.series.profit', "Profit"); st.subheader(prof_title_trans)
                prof_data = data.get('profit_data'); df_prof = pd.DataFrame(prof_data) if prof_data and isinstance(prof_data, list) else pd.DataFrame()
                ...
                else: st.info(f"{get_translation_streamlit(current_lang, 'error.no_data', 'No Data Available')} ({prof_title_trans})")
                return None # Handled directly
            else: # Single Pie (like fig2)
                 ...
                 else: st.info(f"{title}: {get_translation_streamlit(current_lang, 'error.no_data', 'No Data Available')}"); return None # Handled

        elif isinstance(data, list):
            # --- DataFrame Plotting Logic ---
            if not data: st.info(f"{title}: {get_translation_streamlit(current_lang, 'error.no_data', 'No Data Available')}"); return None
            df = pd.DataFrame(data)
            if df.empty: st.info(f"{title}: {get_translation_streamlit(current_lang, 'error.no_data', 'No Data Available')}"); return None
            ...
            x_col_actual = find_column(df, [x_title_trans, x_name_internal, 'date'], title)
            if not x_col_actual:
                if df.columns.any(): x_col_actual = df.columns[0]; fig.update_layout(xaxis_title=x_col_actual)
                else: raise ValueError(f"DataFrame for {fig_key} has no columns.")

            # --- Specific Figure Logic (based on V5.3) ---
            if fig_key == 'fig1':
                 ...
                 if x_col_actual and y_col_stock and y_col_sales: fig.add_trace(go.Bar(x=df[x_col_actual], y=df[y_col_stock], name=trans_current_stock, marker_color='rgb(55, 83, 109)')); fig.add_trace(go.Bar(x=df[x_col_actual], y=df[y_col_sales], name=trans_sales, marker_color='rgb(26, 118, 255)')); fig.update_layout(barmode="group", yaxis_title=trans_quantity)
                 ...
                 if x_col_actual and y_col_cogs and y_col_revenue and y_col_margin: fig = px.bar(df, x=x_col_actual, y=[y_col_cogs, y_col_revenue, y_col_margin], barmode="group", title=title, labels={'value': trans_amount, 'variable': get_translation_streamlit(current_lang, 'common.type', 'Type'), x_col_actual: x_title_trans}, color_discrete_map={y_col_cogs: "rgb(255, 127, 14)", y_col_revenue: "rgb(26, 118, 255)", y_col_margin: "rgb(44, 160, 44)"});
                 ...
                 y_col_qty = find_column(df, [trans_qty_sold, 'sales_quantity', 'Quantity Sold'], title)
                 ...
                 if not x_col_price: raise ValueError(f"Could not find X-axis (Sale Price) column for fig7.")
                 # --- *** Corrected Hover Name Logic for Fig7 *** ---
                 # Search for the product name column specifically, excluding x and y
                 # Use the standard translated key 'Product Name' first, then 'name', etc.
                 col_name_hover = find_column(df, [trans_product_name_hover, 'name', trans_product], title, exclude=[x_col_price, y_col_qty])
                 if not col_name_hover:
                     logger.warning(f"[{fig_key}] Could not find a distinct product name column for hover_name. Hover name disabled.")
                     hover_name_param = None # Explicitly set hover_name to None if column not found
                 else:
                      hover_name_param = col_name_hover # Use the found name column
                 # --- *** End Corrected Hover Name Logic *** ---
                 fig = px.scatter(df,
                                  x=x_col_price,
                                  y=y_col_qty,
                                  title=title,
                                  labels={x_col_price: x_title_trans, y_col_qty: y_title_trans},
                                  hover_name=hover_name_param # Use the found name column here
                                  # hover_data is removed - px.scatter includes x, y, and hover_name by default
                                  )
            elif fig_key == 'fig9':
                ...
                logger.warning(f"No specific DataFrame plotting logic for {fig_key}. Applying default settings.");
                if len(df.columns) >= 2:
                    y_col_default = find_column(df, df.columns.tolist(), title, exclude=x_col_actual)
                    if y_col_default: logger.info(f"Default plot: X='{x_col_actual}', Y='{y_col_default}'"); fig.add_trace(go.Scatter(x=df[x_col_actual], y=df[y_col_default], mode='lines+markers', name=y_col_default))
                    else: logger.warning(f"Could not find a suitable Y column for default plot for {fig_key}.")
                else: logger.warning(f"Cannot create default plot for {fig_key}, less than 2 columns.")

            # --- Final Checks and Return for DataFrame charts ---
            if fig and fig_key != 'fig14': # Ensure fig exists and is not the table case
                 if description: fig.update_layout(annotations=[dict(text=description, showarrow=False, xref='paper', yref='paper', x=0.5, y=-0.15, xanchor='center', yanchor='top', align='center')])
                 if not fig.data: # Check if any traces were actually added
                      logger.warning(f"[{fig_key}] Figure created, but no data traces added.");
                      st.warning(f"{title}: {get_translation_streamlit(current_lang,'error.no_series_plotted','No data series could be plotted')}")
                      return None # Handled by warning message
                 logger.info(f"Successfully created DataFrame-based figure object for key: {fig_key}")
                 return fig # Return the figure to be plotted
            elif fig_key != 'fig14': # If fig is None but not fig14 (e.g., error during specific plot logic)
                 logger.error(f"[{fig_key}] Figure object became None unexpectedly during DataFrame plotting.")
                 st.error(f"{title}: {get_translation_streamlit(current_lang, 'error.calculation_error', 'Error creating chart.')}")
                 return None # Handled by error message

        elif isinstance(data, dict): # Handle non-pie, non-error dictionaries (e.g., table)
            if chart_type_str == ChartType.TABLE.value:
                 logger.info(f"[{fig_key}] Handling table data dictionary.")
                 # Attempt to extract data, assuming it might be nested or direct list
                 table_list = data.get('data', []) if isinstance(data.get('data'), list) else []
                 if not table_list and isinstance(data, list): table_list = data # Handle older format?
                 if table_list:
                     df_table = pd.DataFrame(table_list)
                     if not df_table.empty: st.subheader(title); st.dataframe(df_table, use_container_width=True)
                     else: st.info(f"{title}: {get_translation_streamlit(current_lang, 'error.no_data', 'No Data Available')}")
                 else: st.info(f"{title}: {get_translation_streamlit(current_lang, 'error.no_data', 'No Data Available')}")
                 return None # Handled directly
            else:
                 logger.warning(f"[{fig_key}] Received unhandled dictionary data format for chart type '{chart_type_str}'.");
                 st.error(f"Cannot display {fig_key} ({title}): Unsupported dictionary format."); return None

        else: # Should not happen if initial checks are correct
             logger.error(f"[{fig_key}] MAJOR ERROR: Reached end with unexpected data type: {type(data)}");
             st.error(f"Cannot display {fig_key} ({title}): Critical error with data format."); return None

    except ValueError as ve: # Catch ValueErrors specifically raised during plotting logic
        st.error(f"Data Error creating '{fig_key}' ('{title}'): {ve}")
        logger.error(f"ValueError creating '{fig_key}': {ve}", exc_info=False) # Log less verbosely for ValueErrors
        return None # Handled by error message
    except Exception as e: # Catch any other unexpected error during plotting
        st.error(f"Critical plotting error for '{fig_key}' ('{title}'): {e}")
        logger.critical(f"Critical error creating '{fig_key}': {e}", exc_info=True)
        return None # Handled by error message
...
# --- Client and Language Selection ---
col_select1, col_select2 = st.columns([1, 1])
with col_select1:
    available_clients = get_clients_from_db(pipeline)
    if not available_clients: st.error("Could not fetch client list from database. Dashboard cannot proceed."); st.stop()
    ...
    try: selected_client_id = int(selected_client_str) if selected_client_str else None
    except ValueError: st.error("Invalid Client ID selected."); selected_client_id = None

with col_select2:
    lang_options = SUPPORTED_LANGUAGES; default_lang_index = 0
    if 'ar' in lang_options: default_lang_index = lang_options.index('ar')
    elif 'en' in lang_options: default_lang_index = lang_options.index('en')
    selected_lang = st.selectbox("Select Language:", lang_options, index=default_lang_index, key="lang_sel_db")

# --- Define Figure Frequencies (Original V5.3 figures) ---
...
if selected_client_id is not None:
    ...
    logger.info(f"Displaying dashboard V{app_version} - Client: {selected_client_id}, Lang: {selected_lang}")

    # Get the prepared data (ChartData objects, possibly with errors)
    all_prepared_data = run_pipeline_and_get_data(pipeline, selected_client_id, selected_lang)

    if all_prepared_data is None:
        # Error message already shown by run_pipeline_and_get_data
        st.warning("Data processing failed. Please check logs or try again later.")
    else:
        # --- Setup Tabs ---
        ...
        tab_daily, tab_weekly, tab_monthly, tab_quarterly = st.tabs([
            f" {tab_titles['daily']}", f" {tab_titles['weekly']}", f" {tab_titles['monthly']}", f" {tab_titles['quarterly']}"
        ])
        tabs_config = { "daily": tab_daily, "weekly": tab_weekly, "monthly": tab_monthly, "quarterly": tab_quarterly }

        # --- Organize Figures by Frequency ---
        ...
        for fig_key, chart_data_obj in all_prepared_data.items():
            freq = FIGURE_FREQUENCY_MAP.get(fig_key) # Use the map without fig8
        ...
        # --- Loop through tabs and display figures OR errors ---
        for freq, tab in tabs_config.items():
            with tab:
                st.subheader(f"{tab_titles[freq]} Reports")
                figures_for_tab = figures_by_frequency[freq]
                if not figures_for_tab: st.info(f"{get_translation_streamlit(selected_lang, 'error.no_data', 'No data available')} for {freq} reports."); continue

                figures_processed = 0
                figure_keys_sorted = sorted(figures_for_tab.keys(), key=lambda x: int(x.replace('fig','')) if x.startswith('fig') and x[3:].isdigit() else 99)
                num_cols = 2; cols = st.columns(num_cols); col_index = 0

                for fig_key in figure_keys_sorted:
                    chart_data_obj = figures_for_tab.get(fig_key)
                    with cols[col_index % num_cols]:
                        if chart_data_obj:
                            try:
                                # This function now handles plotting OR displaying the error msg
                                plotly_fig = create_plotly_figure_replica(fig_key, chart_data_obj, selected_lang)
                                if plotly_fig: # If a figure object was returned, display it
                                     st.plotly_chart(plotly_fig, use_container_width=True)
                                     figures_processed += 1; col_index += 1
                                # If None was returned, function handled display (error/table/info)
                                elif plotly_fig is None:
                                     figures_processed += 1; col_index += 1
                                else: # Should not happen
                                     logger.error(f"Unexpected return type from plotter for {fig_key}.")
                                     st.error(f"Failed to render {fig_key}: Internal display error.")
                                     col_index += 1
                            except Exception as e_plot:
                                 st.error(f"Failed to process display for {fig_key}: {e_plot}")
                                 logger.error(f"Error rendering {fig_key} (Freq: {freq}): {e_plot}", exc_info=True)
                                 col_index += 1
                        else: # chart_data_obj is None
                             st.warning(f"Could not load data structure for: {fig_key}")
                             logger.warning(f"ChartData object was None in dict for {fig_key} (Freq: {freq})")
                             col_index += 1

                if figures_processed == 0 and figures_for_tab:
                    st.warning(f"Could not display or report issues for any visualizations for {freq} frequency.")
                    logger.warning(f"No figures processed/rendered for freq '{freq}', client {selected_client_id}, lang {selected_lang}, although figures were expected.")
...
st.sidebar.caption(f"Dashboard Version: {app_version}") # Use variable

logger.info(f"--- Streamlit dashboard V{app_version} script execution finished ---")