...
try:
    ...
    FORECAST_DATE_COL_MONTHLY = 'date'
    ...
    def _validate_input_data(self):
        ...
        for table in required_tables:
            ...
            if df.empty:
                ...
                # Don't fail validation for empty tables, but check columns if not empty
                if not df.empty: # Proceed to check columns only if not empty
                     ...
                     if missing:
                         ...
                         all_data_valid = False
            else: # DataFrame is not empty, check columns
                 req_cols = config.required_columns.get(table)
                 if not req_cols:
                     logger.warning(f"No required columns defined for table '{table}' in config. Skipping column check for this table.")
                     continue
